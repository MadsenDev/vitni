...
export function TimelineWorkspace({ nodes, edges }: TimelineWorkspaceProps) {
  ...
  const name = (n: GraphNode | undefined) => n?.label || 'Unknown';
  ...
  const events = useMemo<TimelineEvent[]>(() => {
    ...
    for (const n of nodes) {
      if (n.type === 'person') {
        ...
        if (dob) out.push({ id: `birth:${n.id}`, date: dob, sortKey: Date.parse(dob) || 0, text: `${name(n)} was born ${formatDate(dob)}.` });
      }
    }
    ...
    for (const e of edges) {
      ...
      out.push({ id: e.id, date, sortKey: Date.parse(date) || 0, text: relationText(e.type, subtype, A, B, date) });
    }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-slate-800 bg-slate-950/80 px-6 py-3">
        <h1 className="text-lg font-semibold text-white">Timeline Workspace</h1>
        <div className="text-sm text-slate-400">Events: {events.length}</div>
      </div>
